...
#include <afxext.h>
#include "..\PDFReader\PDFReaderDoc.h"
#include "..\PDFReader\PDFReaderView.h"
...
protected:
	...
	DECLARE_DOCUMENT(CPDFReaderDoc)
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CPDFReaderView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CPDFReaderDoc, (CObject*) m_pDocument->GetContainer());

...
{
public:
	CHE_PDF_NameTree( CHE_PDF_DictionaryPtr dictPtr, CHE_Allocator * pAllocator = nullptr ) : CHE_Object( pAllocator ), mDictPtr( dictPtr ) {};

	CHE_PDF_ObjectPtr GetObject( const CHE_ByteString & name );

private:
	bool Find( const string & name, const CHE_PDF_DictionaryPtr & dict, CHE_PDF_ObjectPtr & objRet );

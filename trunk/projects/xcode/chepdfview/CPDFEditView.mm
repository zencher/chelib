...
- (id)initWithFrame:(NSRect)frameRect
{
    ...
    if (self) {
        ...
        thumbnailView = [[CPDFThumbnailView alloc] initWithFrame:[leftBarView frame]];
        outlineView = [[CPDFOutlineView alloc] initWithFrame:[rightBarView frame]];
        pageView = [[CPDFPageView alloc] initWithFrame:[middleView frame]];
...
- (NSInteger)numberOfChildren
{
    ...
    {
        NSInteger count = 0;
        CPDF_OutlineItem * item = outlineItem->mpFirst;
        while (item) {
            ++count;
            item = item->mpNext;
        }

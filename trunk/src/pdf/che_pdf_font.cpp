	...
	: CHE_Object(pAllocator), mType(FONT_ENCODING_NONE), mBaseType(FONT_ENCODING_NONE), mpCodeTable(nullptr)
{
	...
	if ( !objPtr )
	{
        CHE_ByteString typeStr( GetAllocator() );
        CHE_ByteString baseFontStr( GetAllocator() );
	...
	mFontFileSize(0), mPlatformFontInfo(nullptr), mCleanCallBack(nullptr), mbBase14Font(false), mFTFace(nullptr)
{
	...
	if ( objPtr )
	{
		...
		mpFontDescriptor = GetAllocator()->New<CHE_PDF_FontDescriptor>(mFontDescDict, GetAllocator());
	}else{
		//Type0 Font
...
PDF_FONT_TYPE CHE_PDF_Font::GetFontType() const
{
	return mFontType;
}
...
CHE_PDF_DictionaryPtr CHE_PDF_Font::GetFontDescDict() const
{

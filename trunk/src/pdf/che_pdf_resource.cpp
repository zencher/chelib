...
HE_BOOL GetCodeFromName( CHE_ByteString & name, HE_BYTE encoding, HE_BYTE & ret )
{
	if ( encoding == PDFENCODING_STANDARD )
	{
		switch ( name[0] )
		{
		case 'A':
	...
	}else if ( encoding == PDFENCODING_WINANSI )
	{
		switch ( name[0] )
		{
		case 'A':
	...
	}else if ( encoding == PDFENCODING_MACROMAN || encoding == PDFENCODING_MACEXPERT )
	{
		switch ( name[0] )
		{
		case 'A':
	...
	}else if ( encoding == PDFENCODING_PDFDOC )
	{
		switch ( name[0] )
		{
		case 'A':
...
CHE_PDF_FontCharCodeMgr::CHE_PDF_FontCharCodeMgr( CHE_PDF_Page * pPage, CHE_PDF_Dictionary * pFontDict )
{
	...
	if ( ((CHE_PDF_Name*)pTmpObj)->GetString() != "Font" )
	{
		return;
	}
...
CHE_PDF_FontCharCodeMgr::~CHE_PDF_FontCharCodeMgr()
{
}

HE_WCHAR CHE_PDF_FontCharCodeMgr::GetUnicode( HE_BYTE ch )
{
	return m_pUnicodeTable[ch];
}
	
HE_WCHAR CHE_PDF_FontCharCodeMgr::GetUnicodeW( HE_WCHAR wch )
{
	return m_pUnicodeTable[(HE_BYTE)wch];
}
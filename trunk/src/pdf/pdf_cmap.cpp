#include "../../include/pdf/pdf_cmap.h"
...
#include "pdf_cmap_res_cns.cpp"
#include "pdf_cmap_res_gb.cpp"
#include "pdf_cmap_res_japan.cpp"
#include "pdf_cmap_res_korea.cpp"
...
CPDF_CMap * CPDF_CMap::LoadBuildinCMap( const CByteString & cmapName,  CAllocator * pAllocator )
{
	...
	if ( pAllocator )
	{
		return pAllocator->New<CPDF_CMap>( pRet, false, pAllocator );
	}
	return new CPDF_CMap( pRet, false, nullptr ); 
}
...
bool CPDF_CMap::LookupCode( uint32 code, uint32 & codeRet ) const
{
	if ( mpCMap )
	{
...
bool CPDF_CMap::IsCode( uint32 cpt, BYTE byteCount ) const
{
	if ( !mpCMap )
	{
		return false;

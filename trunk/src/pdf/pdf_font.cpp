...
bool HE_GetCodeFromName( CByteString & name, BYTE encoding, BYTE & ret )
{
...
uint32 HE_HexStrToValue(const CByteString & str)
{
	uint32 valRet = 0;
	BYTE tmpByte = 0;
...
bool CPDF_Encoding::IsPredefinedCJKCMapNames(const CByteString & name)
{
...
CPDF_Encoding::CPDF_Encoding( const CPDF_DictionaryPtr & fontDict, CAllocator * pAllocator /*= nullptr*/ )
	: CObject(pAllocator), mType(FONT_ENCODING_NONE), mBaseType(FONT_ENCODING_NONE), mpCodeTable(nullptr)
{
	if ( !fontDict )
	{
		return;
	...
	if ( !objPtr )
	{
        CByteString typeStr( GetAllocator() );
        CByteString baseFontStr( GetAllocator() );
        
        CPDF_ObjectPtr objPtr = fontDict->GetElement( "Subtype", OBJ_TYPE_NAME );
        if ( objPtr )
        {
            typeStr = objPtr->GetNamePtr()->GetString();
        }
        objPtr = fontDict->GetElement( "BaseFont", OBJ_TYPE_NAME );
        if ( objPtr )
        {
            baseFontStr = objPtr->GetNamePtr()->GetString();
        }
        
        if ( typeStr == "Type1" && (baseFontStr == "Symbol" || baseFontStr == "ZapfDingbats") )
        {
            if ( baseFontStr == "Symbol" )
            {
                mBaseType = mType = FONT_ENCODING_SYMBOL;
            }else{
                mBaseType = mType = FONT_ENCODING_ZAPFDINGBAT;
            }
            return;
        }else{
            mBaseType = mType = FONT_ENCODING_NONE;
            return;
        }
	}
	...
	}else if ( objPtr->GetType() == OBJ_TYPE_DICTIONARY )
	{
		...
		CPDF_DictionaryPtr pEncodingDict = objPtr->GetDictPtr();
		...
		CPDF_ArrayPtr pDifArray = pEncodingDict->GetElement( "Differences", OBJ_TYPE_ARRAY )->GetArrayPtr();
		if ( pDifArray )
		{
			...
			for ( size_t i = 0; i < iCount; i++ )
			{
				...
				{
					CByteString strTmp = pObj->GetNamePtr()->GetString();
...
CPDF_Encoding::~CPDF_Encoding()
{
	if ( mpCodeTable )
	{
		GetAllocator()->DeleteArray(mpCodeTable);
	}
}


PDF_FONT_ENCODING CPDF_Encoding::GetType() const
{
	return mType;
}

PDF_FONT_ENCODING CPDF_Encoding::GetBaseType() const
{
    return mBaseType;
}

bool CPDF_Encoding::GetUnicode( BYTE charCode, wchar_t & codeRet ) const
{
	if ( mpCodeTable )
	{
	...
	{
	case FONT_ENCODING_STANDARD:
		codeRet = gStandardEncoding[charCode];
		break;
	case FONT_ENCODING_PDFDOC:
		codeRet = gPdfDocEncoding[charCode];
		break;
	case FONT_ENCODING_WINANSI:
		codeRet = gWinAnsiEncoding[charCode];
		break;
	case FONT_ENCODING_MACROMAN:
		codeRet = gMacRomanEncoding[charCode];
		break;
	case FONT_ENCODING_MACEXPERT:
		codeRet = gMacExpertEncoding[charCode];
		break;
	case FONT_ENCODING_SYMBOL:
		codeRet = gSymbolEncoding[charCode];
		break;
	case FONT_ENCODING_ZAPFDINGBAT:
		codeRet = gPdfZapfDingbatsEncoding[charCode];
		break;
...
PDF_FONT_LANGUAGE CPDF_Encoding::GetLanguage( const CByteString & encodingName )
{
...
CPDF_FontDescriptor::CPDF_FontDescriptor( const CPDF_DictionaryPtr & fontDesDict, CAllocator * pAllocator /*= nullptr*/ )
	: CObject( pAllocator ), mFlags( 0 ),  mItalicAngle( 0 ), mAscent( 0 ), mDescent( 0 ), mLeading(0),
	mCapHeight( 0 ), mXHeight( 0 ), mStemV(0), mStemH(0), mMissingWidth( 0 ), mEmbedded( false ), mWMode( 0 )
{
	if ( fontDesDict )
	{
		CPDF_ObjectPtr objPtr = fontDesDict->GetElement( "Flags", OBJ_TYPE_NUMBER );
		if ( objPtr )
		{
...
CPDF_FontDescriptor::~CPDF_FontDescriptor()
{

}


bool CPDF_FontDescriptor::IsFixedPitch() const
{
	return ( (mFlags & 1) > 0 );
}


bool CPDF_FontDescriptor::IsSerif() const
{
	return ( (mFlags & 2) > 0 );
}


bool CPDF_FontDescriptor::IsSymbolic() const
{
	return ( (mFlags & 4) > 0 );
}


bool CPDF_FontDescriptor::IsScript() const
{
	return ( (mFlags & 8) > 0 );
}


bool	CPDF_FontDescriptor::IsNonsymbolic() const
{
	return ( (mFlags & 20) > 0 );
}


bool	CPDF_FontDescriptor::IsItalic() const
{
	return ( (mFlags & 40) > 0 );
}


bool	CPDF_FontDescriptor::IsAllCap() const
{
	return ( (mFlags & 10000) > 0 );
}


bool	CPDF_FontDescriptor::IsSmallCap() const
{
	return ( (mFlags & 20000) > 0 );
}


bool	CPDF_FontDescriptor::IsForceBold() const
{
	return ( (mFlags & 40000) > 0 );
}


CPDF_FontPtr CPDF_Font::Create( const CPDF_DictionaryPtr & fontDict, CAllocator * pAllocator /*= nullptr*/ )
{
    CPDF_FontPtr fontPtr;
	...
	if ( fontDict )
	{
		...
		if ( objPtr )
		{
			CByteString str( pAllocator );
			str = objPtr->GetNamePtr()->GetString();
			if ( str == "Type0" )
			{
				pFont = pAllocator->New<CPDF_Type0_Font>( fontDict, pAllocator );
			}
			else if ( str == "Type1"  )
			{
				pFont = pAllocator->New<CPDF_Type1_Font>( fontDict, pAllocator );
			}
			else if ( str == "MMType1" )
			{
				pFont = pAllocator->New<CPDF_MMType1_Font>( fontDict, pAllocator );
			}
			else if ( str == "Type3" )
			{
				pFont = pAllocator->New<CPDF_Type3_Font>( fontDict, pAllocator );
			}
			else if ( str == "TrueType" )
			{
				pFont = pAllocator->New<CPDF_TrueType_Font>( fontDict, pAllocator );
			}
		}
	}
    if (pFont)
...
CPDF_Font::CPDF_Font( const CPDF_DictionaryPtr & fontDict, CAllocator * pAllocator /*= nullptr*/ )
: CPDF_Component(COMPONENT_TYPE_Font, CPDF_ObjectPtr(), pAllocator), mFontType(FONT_TYPE1), mBaseFont(pAllocator), 
...
{
	CPDF_ObjectPtr objPtr = mFontDict->GetElement( "Subtype", OBJ_TYPE_NAME );
	if ( objPtr )
	{
		...
		{
			mFontType = FONT_TYPE1;
		}else if ( str == "MMType1" )
		{
			mFontType = FONT_MMTYPE1;
		}else if ( str == "Type3" )
		{
			mFontType = FONT_TYPE3;
		}else if ( str == "TrueType" )
		{
			mFontType = FONT_TRUETYPE;
		}
	}
	...
	if (objPtr)
	{
		ParseToUnicodeStream(objPtr->GetStreamPtr());
	}

	objPtr = mFontDict->GetElement("FontDescriptor", OBJ_TYPE_DICTIONARY);
	if ( objPtr )
	{
		mFontDescDict = objPtr->GetDictPtr();
		mpFontDescriptor = GetAllocator()->New<CPDF_FontDescriptor>(mFontDescDict, GetAllocator());
	}else{
		//Type0 Font
	...
	if ( mpFontDescriptor )
	{
		CPDF_ReferencePtr refPtr = mpFontDescriptor->GetEmbeddedStream();
		if ( refPtr )
		{
...
CPDF_Font::~CPDF_Font()
{
	if ( mpFontDescriptor )
	{
		mpFontDescriptor->GetAllocator()->Delete<CPDF_FontDescriptor>( mpFontDescriptor );
...
PDF_FONT_TYPE CPDF_Font::GetFontType() const
{
	return mFontType;
}


CByteString CPDF_Font::GetBaseFont() const
{
	return mBaseFont;
}


PDF_FONT_ENCODING CPDF_Font::GetEncodingType() const
{
	return mEncoding.GetType();
}

PDF_FONT_LANGUAGE CPDF_Font::GetLanguage() const
{
    return mLanguage;
}

CPDF_DictionaryPtr CPDF_Font::GetFontDict() const
{
	return mFontDict;
}


CPDF_DictionaryPtr CPDF_Font::GetFontDescDict() const
{
	return mFontDescDict;
}

uint32 CPDF_Font::GetWMode() const
{
	if (mpFontDescriptor)
	{
...
uint32 CPDF_Font::GetFontDesender()
{
#ifdef _MAC_OS_X
...
uint32 CPDF_Font::GetFontAscender()
{
#ifdef _MAC_OS_X
    CTFontRef ctfontRef = CTFontCreateWithGraphicsFont((CGFontRef)mPlatformFontInfo, 1, nil, nil);
...
void CPDF_Font::ParseToUnicodeStream(const CPDF_StreamPtr & stm)
{
	if (!stm)
	{
		return;
	...
	IRead * pFileRead = CreateMemBufIRead((BYTE*)(stmAcc.GetData()), stmAcc.GetSize(), GetAllocator());
	CPDF_SyntaxParser parser( nullptr, GetAllocator() );
		...
		if ( wordDes.type == PARSE_WORD_INTEGER )
		{
			...
			if ( parser.IsWord( "beginbfchar" ) )
			{
				for ( size_t i = 0; i < lCount; i++ )
				{
					...
					parser.GetWord( wordDes );
                    mToUnicode.insert( std::pair<uint32,uint32>( lIndex, HE_HexStrToValue( parser.GetString() ) ) );
					lCodeCount++;
				}
			}else if ( parser.IsWord( "beginbfrange" ) )
			{
				for ( size_t j = 0; j < lCount; j++ )
				{
					...
					if ( wordDes.type == PARSE_WORD_ARRAY_B )
					{
						...
						CPDF_ArrayPtr pArray = parser.GetArrayPtr();
						if ( ! pArray )
						{
							continue;
						}
						CPDF_StringPtr pStrObj;
						...
						for ( size_t i = lIndex ; i <= lIndexEnd; i++ )
						{
							...
							CByteString strTmp = pStrObj->GetString();
                            mToUnicode.insert(std::pair<uint32, uint32>(i, HE_HexStrToValue(strTmp)));
							lCodeCount++;
						}
					}else if ( wordDes.type == PARSE_WORD_STRING )
					{
						...
						for ( size_t i = lIndex ; i <= lIndexEnd; i++ )
						{
							mToUnicode.insert(std::pair<uint32, uint32>(i, val));
							lCodeCount++;
							tmpValue++;
							val++;
						}
...
CPDF_SimpleFont::CPDF_SimpleFont(const CPDF_DictionaryPtr & fontDict, CAllocator * pAllocator /*= nullptr*/)
	: CPDF_Font(fontDict, pAllocator), mFirstChar(0), mLastChar(255)
{
	memset(mCharWidths, 0, sizeof(int32)* 256);
	memset(mGlyphId, 0, sizeof(wchar_t)* 256);

	CPDF_ObjectPtr objPtr = mFontDict->GetElement("FirstChar", OBJ_TYPE_NUMBER);
	if (objPtr)
	{
...
CPDF_Type1_Font::CPDF_Type1_Font( const CPDF_DictionaryPtr & pFontDcit, CAllocator * pAllocator /*= nullptr*/ )
	: CPDF_SimpleFont(pFontDcit, pAllocator)
{
	if (mPlatformFontInfo)
	{
		...
		CPDF_DictionaryPtr encodingDict;
		CPDF_ObjectPtr objPtr = mFontDict->GetElement("Encoding", OBJ_TYPE_DICTIONARY);
		if (objPtr)
		{
			encodingDict = objPtr->GetDictPtr();
			CPDF_ArrayPtr pDifArray = encodingDict->GetElement("Differences", OBJ_TYPE_ARRAY)->GetArrayPtr();
			if (pDifArray)
			{
				size_t iCount = pDifArray->GetCount();
				size_t iIndex = 0;
...
CPDF_Type1_Font::CPDF_Type1_Font(int noType1Handle, const CPDF_DictionaryPtr & pFontDcit, CAllocator * pAllocator/* = nullptr*/)
	:CPDF_SimpleFont(pFontDcit, pAllocator)
{
}


CPDF_Type1_Font::~CPDF_Type1_Font()
{
}


bool CPDF_Type1_Font::Decode(wchar_t charCode, wchar_t & ucs, uint32 & gid, uint32 & cid) const
{
	bool bUCSGet = false;
    bool bGIDGet = false;
	cid = 0;
    ...
    if ( mEncoding.GetUnicode(charCode, ucs) )
    {
        bUCSGet = true;
    }
...
FLOAT CPDF_Type1_Font::GetWidth( const CPDF_TextItem & item, const CMatrix & matrix /*= CMatrix()*/ )
{
	CMatrix tmpMatrix;
	tmpMatrix.a = 0;
	tmpMatrix.b = 0;
...
CPDF_MMType1_Font::CPDF_MMType1_Font( const CPDF_DictionaryPtr & pFontDict, CAllocator * pAllocator /*= nullptr*/ )
	: CPDF_Type1_Font(pFontDict, pAllocator)
{
}


CPDF_MMType1_Font::~CPDF_MMType1_Font()
{
}


CPDF_TrueType_Font::CPDF_TrueType_Font( const CPDF_DictionaryPtr & pFontDict, CAllocator * pAllocator /*= nullptr*/ )
	: CPDF_Type1_Font(1, pFontDict, pAllocator)
{
    if (mFTFace)
    {
    ...
    if (mFTFace)
    {
        ...
        if (mpFontDescriptor && !mpFontDescriptor->IsSymbolic() && ((FT_Face)mFTFace)->charmap && ((FT_Face)mFTFace)->charmap->platform_id == 3)
        {
            for (size_t i = 0; i < 256; ++i)
            {
                if (mGlyphNames[i].GetLength())
                {
                    int aglcode = pdf_lookup_agl((char*)(mGlyphNames[i].GetData()));
                    if (!aglcode)
                        mGlyphId[i] = FT_Get_Name_Index(((FT_Face)mFTFace), (char*)(mGlyphNames[i].GetData()));
                    //else
                    //mCIDTOGID[i] = ft_char_index(((FT_Face)mFTFace), aglcode);
                }
            }
        }
        ...
        //            if (k <= 0)
        //                mCIDTOGID[i] = FT_Get_Name_Index(((FT_Face)mFTFace), (char*)(estrings[i].GetData()));
        //            //else
        //            //    mCIDTOGID[i] = ft_char_index(((FT_Face)mFTFace), k);
        //        }
        //    }
        //}
        ...
        else if (!((FT_Face)mFTFace)->charmap || ((FT_Face)mFTFace)->charmap->encoding != FT_ENCODING_MS_SYMBOL)
        {
            for (size_t i = 0; i < 256; i++)
            {
                if (mGlyphNames[i].GetData())
                {
                    mGlyphId[i] = FT_Get_Name_Index(((FT_Face)mFTFace), (char*)(mGlyphNames[i].GetData()));
                    //if (mCIDTOGID[i] == 0)
                    //    mCIDTOGID[i] = ft_char_index(((FT_Face)mFTFace), i);
                }
            }
        }
        ...
        for (size_t i = 0; i < 256; i++)
        {
            ...
            {
                if (FT_HAS_GLYPH_NAMES(((FT_Face)mFTFace)))
                {
                    FT_Error err = FT_Get_Glyph_Name(((FT_Face)mFTFace), mGlyphId[i], tempStr, 32);
                    if (!err)
                    {
                        mGlyphNames[i] = tempStr;
                    }
...
CPDF_TrueType_Font::~CPDF_TrueType_Font()
{
}

bool CPDF_TrueType_Font::Decode( wchar_t charCode, wchar_t & ucs, uint32 & gid, uint32 & cid ) const
{
    bool bUCSGet = false;
    bool bGIDGet = false;
    cid = 0;
    ...
    {
        FT_Face ftface = (FT_Face)mFTFace;
        gid = FT_Get_Char_Index( ftface, charCode );
        if ( gid == 0 )
        {
            CTFontRef ctFontRef  = CTFontCreateWithGraphicsFont((CGFontRef)mPlatformFontInfo, 1, nil, nil);
...
CPDF_Type3_Font::CPDF_Type3_Font( const CPDF_DictionaryPtr & pFontDict, CAllocator * pAllocator /*= nullptr*/ )
	: CPDF_Font(pFontDict, pAllocator), mFirstChar(0), mLastChar(255)
{
	if ( pFontDict )
	{
		CPDF_ObjectPtr objPtr = pFontDict->GetElement( "FontBBox", OBJ_TYPE_ARRAY );
		if ( objPtr )
		{
		...
		CPDF_ArrayPtr encodingDiffArray;
...
CPDF_Type3_Font::~CPDF_Type3_Font()
{
    std::unordered_map<uint32, CPDF_ContentObjectList*>::iterator it = mGlyphMap.begin();
...
CPDF_ArrayPtr CPDF_Type3_Font::GetFontBBox() const
{
	return mFontBBox;
}


CMatrix CPDF_Type3_Font::GetFontMatrix() const
{
	return mFontMatrix;
}


CPDF_DictionaryPtr CPDF_Type3_Font::GetResDict() const
{
	return mResDict;
}


CPDF_ContentObjectList* CPDF_Type3_Font::GetGlyph(BYTE charCode)
{
    if (charCode>= mFirstChar && charCode <= mLastChar)
    {
        std::unordered_map<uint32, CPDF_ContentObjectList*>::iterator it = mGlyphMap.end();
        it = mGlyphMap.find(charCode);
        if ( it != mGlyphMap.end() )
        {
            return it->second;
        }else{
            CPDF_ReferencePtr refPtr = mCharCodeToRef[charCode-mFirstChar];
            if (refPtr) {
                CPDF_ContentObjectList * pContentList = GetAllocator()->New<CPDF_ContentObjectList>(GetAllocator());
                if ( CPDF_ContentListBuilder::ParseContentStream(refPtr, *pContentList, nullptr)) {
...
bool CPDF_Type3_Font::Decode(wchar_t charCode, wchar_t & ucs, uint32 & gid, uint32 & cid) const
{
	return false;
}

FLOAT CPDF_Type3_Font::GetWidth(const CPDF_TextItem & item, const CMatrix & matrix /*= CMatrix()*/)
{
	return 1.0f;
}
...
CPDF_Type0_Font::CPDF_Type0_Font(const CPDF_DictionaryPtr & fontDict, CAllocator * pAllocator /*= nullptr*/)
: CPDF_Font(fontDict, pAllocator), mpCIDMap(nullptr), mpUnicodeMap(nullptr), mCIDTOGID(nullptr), mCIDTOGIDLength(0), mDefaultWidth(1000)
{
	if (mFontDict)
	{
		CPDF_ObjectPtr objPtr;
		...
		if (GetEncodingType() == FONT_ENCODING_BUILDINCMAP)
		{
			...
			if (objPtr)
			{
				CByteString str(pAllocator);
				str = objPtr->GetNamePtr()->GetString();
				mpCIDMap = CPDF_CMap::LoadBuildinCMap(str, GetAllocator());
                mLanguage = CPDF_Encoding::GetLanguage(str);
			}
		}
        ...
        if (mDescdtFontDict)
        {
			...
			if (objPtr)
			{
				CPDF_DictionaryPtr pCIDSystemInfoDict = objPtr->GetDictPtr();
				objPtr = pCIDSystemInfoDict->GetElement("Registry", OBJ_TYPE_STRING);
				if (objPtr)
				{
					CPDF_StringPtr pSt = objPtr->GetStringPtr();
					CByteString cmapNuame(pAllocator);
					...
					if (objPtr)
					{
						...
						mpUnicodeMap = CPDF_CMap::LoadBuildinCMap(cmapNuame, GetAllocator());
					}
				}
			}
		}
...
bool CPDF_Type0_Font::Decode(wchar_t charCode, wchar_t & ucs, uint32 & gid, uint32 & cid) const
{
	...
	if (type == FONT_ENCODING_BUILDINCMAP)
	{
		...
		if (bCidGot && mCIDTOGID && cid < mCIDTOGIDLength)
		{
			gid = mCIDTOGID[cid];
			bGidGot = true;
		}
    ...
    //有些文档没有使用默认的cmap，无法从charcode获得cid，然后再获得gid，但是却有CIDTOGID信息，可以从charcode直接获得gid
    if (!bGidGot && mCIDTOGID && charCode < mCIDTOGIDLength)
    {
...
FLOAT CPDF_Type0_Font::GetWidth(const CPDF_TextItem & item, const CMatrix & matrix /*= CMatrix()*/)
{
	CMatrix tmpMatrix;
	tmpMatrix.a = 0;
	tmpMatrix.b = 0;
...
bool CPDF_Type0_Font::IsCode(uint32 cpt, BYTE byteCount)
{
	if (mpCIDMap)
	{
...
CPDF_Type0_Font::~CPDF_Type0_Font()
{
	if (mpCIDMap)
	{
		mpCIDMap->GetAllocator()->Delete<CPDF_CMap>(mpCIDMap);
	}
	if (mpUnicodeMap)
	{
		mpUnicodeMap->GetAllocator()->Delete<CPDF_CMap>(mpUnicodeMap);
	}
}
